...
// 分区策略执行
export const executeV2 = function (params: {
  bk_biz_id: number;
  partition_infos: {
    cluster_id: number;
    configs: {
...
// 查询分区执行失败日志详情
export const saveAndExecuteV2 = function (params: {
  cluster_id: number;
  dblikes: string[];
  expire_time: number;
  force?: boolean;
...
// 查询分区执行失败日志详情
export const queryFailLog = function (params: { config_id: number }) {
  return http.post<{
    bk_biz_id: number;
    bk_cloud_id: number;
    cluster_type: string;
...
// 根据执行状态过滤分区配置
export const queryConfByStatus = function (params: {
  bk_biz_id: number;
  cluster_type: ClusterTypes;
  limit?: number;
  offset?: number;
  ...
  return http.post<ListBase<PartitionModel[]>>('/apis/partition/query_conf_by_status/', params).then((data) => ({
    ...data,
    results: data.results.map(
      (item) =>
        new PartitionModel(
