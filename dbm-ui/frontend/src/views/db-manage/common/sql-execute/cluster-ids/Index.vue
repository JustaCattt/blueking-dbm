...
<script setup lang="tsx">
  ...
  const fetchStorageEngines = (clusters: IClusterData[]) => {
    const clusterIds = clusters
      .filter((item) => item.cluster_type === ClusterTypes.TENDBHA || item.cluster_type === ClusterTypes.TENDBSINGLE || item.cluster_type === ClusterTypes.TENDBCLUSTER)
      .map((item) => item.id);
    ...
    return getDefaultStorageEngines({ cluster_ids: clusterIds }).then((engineData) => {
      const engineMap = engineData.reduce(
        (result, item) => {
          result[item.cluster_id] = item.default_storage_engine;
          return result;
        },
      ...
      clusters.forEach((item) => {
        if (engineMap[item.id] !== undefined) {
          item.default_storage_engine = engineMap[item.id];
        }
      });
    });
  };
  ...
  const triggerChange = () => {
    ...
    clusterStorageEngines.value = targetClusterList.value.reduce(
      (result, item) => {
        result[item.id] = item.default_storage_engine || '';
        return result;
      },
      {} as Record<number, string>,
    );
