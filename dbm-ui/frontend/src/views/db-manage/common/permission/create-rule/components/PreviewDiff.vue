...
<script setup lang="tsx">
  ...
  const diffArray = (oldArray: string[], newArray: string[]) => {
    const diffMap: Record<string, PrivilegeRow['diffType']> = {}
    oldArray.forEach(item => {
      diffMap[item] = 'delete';
    });
    newArray.forEach(item => {
      if (Object.prototype.hasOwnProperty.call(diffMap, item)) {
        diffMap[item] = 'unchanged';
      } else {
        diffMap[item] = 'add';
      }
    });
  ...
  const getSensitiveWordMap= () => props.ruleSettingsConfig.ddlSensitiveWords?.reduce<Record<string, true>>((acc, cur) => {
      acc[cur] = true;
      return acc;
    }, {}) || {}

  const getPrivilegeDataByType = (key: AuthItemKey) => {
  ...
  const privilegeData = computed(() => [
    ...getPrivilegeDataByType('dml'),
    ...getPrivilegeDataByType('ddl'),
    ...getPrivilegeDataByType('glob'),
  ]);
  ...
  const tableHeight = computed(() => privilegeData.value.length >= 12 ? 550 : undefined);
