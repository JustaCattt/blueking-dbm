...
<script setup lang="ts">
  ...
  const getAllCheckedboxValue = (key: AuthItemKey) =>
    formData.value.privilege[key]?.length === props.ruleSettingsConfig.dbOperations[key].length;

  const topAllCheckedboxValue = computed(() => (['ddl', 'dml', 'glob'] as AuthItemKey[]).every(getAllCheckedboxValue));
  ...
  const topAllCheckedboxIndeterminate = computed(() =>
    (['ddl', 'dml', 'glob'] as AuthItemKey[]).some(getAllCheckedboxIndeterminate),
  );
